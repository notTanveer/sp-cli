import { WITNESS_FLAG, WITNESS_MARKER } from './constants.js';
import { encodingLength, isPubKey, readVarInt } from './utility.js';
...
export class Transaction {
    ...
    static fromBuffer(buffer) {
        ...
        offset += 4;
        const hasWitnesses = buffer.readUInt8(offset) === WITNESS_MARKER &&
        ...
        if (hasWitnesses) {
            for (let i = 0; i < vinLen; i++) {
                ...
                for (let j = 0; j < vectorSize; j++) {
                    ...
                    vector.push(buffer.subarray(offset, offset + witnessItemLen));
